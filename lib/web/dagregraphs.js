  ...
  $("g.node").each(function ()
      {
        ...
        var marker = ' ';
        if(state.marker) {
          marker += String(state.marker.className);
          $(this).click(function(e){
              console.log('Substitutions for node: ');
              console.log('------------------------');
              for(var i = 0; i < state.marker.theta.length; i++){
                console.log('Match number ' + (i + 1) + ':');
                for(var j = 0; j < state.marker.theta[i].length; j++){
                  console.log(state.marker.theta[i][j]);
                }
                
              }
              console.log('------------------------');
              $('div#queryInfo').html(state.marker.info);
          })
          //.mouseout(function(){
          //    $('div#queryInfo').html('Hover over a marked node to see the substitutions.')
          //})
        };
//            constructorName = constructorName.substring(0, constructorName.length);
        $(this).attr("class", "node enter " + constructorName + marker)
          .click(function(e) {
            e.stopPropagation();
